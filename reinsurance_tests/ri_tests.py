...
Coverage = namedtuple("Coverage", "coverage_id tiv")
FmProgramme = namedtuple("FmProgramme", "from_agg_id level_id to_agg_id")
...
FmPolicyTc = namedtuple("FmPolicyTc", "layer_id level_id agg_id policytc_id")
...
FmXref = namedtuple("FmXref", "output_id agg_id layer_id")
...
class DirectLayer:
    ...
    def apply_fm(self, loss_percentage_of_tiv=1.0):
        guls_list = list()
        for item_id, tiv in itertools.izip(self.item_ids, self.item_tivs):
            ...
            guls_list.append(
                GulRecord(event_id=1, item_id=item_id, sidx=1, loss=event_loss))
        guls = pd.DataFrame(guls_list)
        guls.to_csv("guls.csv", index=False)
        command = "gultobin -S 1 < guls.csv | fmcalc | fmtocsv > ils.csv"
        proc = subprocess.Popen(command, shell=True)
        proc.wait()
        if proc.returncode != 0:
        ...
        losses_df = pd.merge(direct_layer.xref_descriptions,
...
losses_df = direct_layer.apply_fm(loss_percentage_of_tiv=0.1)
